...
fn cast_aligned_spv_succeeds() {
    ...
    let words = vulkan_rust::cast_to_u32(&data.0).expect("cast aligned SPIR-V bytes to u32");
...
fn cast_rejects_non_multiple_of_four() {
    ...
    let err = vulkan_rust::cast_to_u32(&data.0).unwrap_err();
    assert_eq!(err, vulkan_rust::BytecodeError::InvalidLength(7));
}

#[test]
fn cast_empty_slice_succeeds() {
    let words = vulkan_rust::cast_to_u32(&[]).expect("cast empty slice to u32");
