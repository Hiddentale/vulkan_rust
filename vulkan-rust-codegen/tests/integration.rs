//! Integration test: runs the generator binary and verifies vulkan-rust-sys compiles.
//!
...
fn generator_runs_successfully() {
    ...
    let output = cargo()
        .args(["run", "-p", "vulkan-rust-codegen"])
        .output()
...
fn generated_output_compiles() {
    ...
    let output = cargo()
        .args(["build", "-p", "vulkan-rust-sys"])
        .output()
        .expect("failed to launch cargo build");
    let stderr = String::from_utf8_lossy(&output.stderr);
    assert!(
        output.status.success(),
        "vulkan-rust-sys failed to compile:\n{stderr}"
    );
}
...
fn generator_output_is_deterministic() {
    ...
    let generated_files: Vec<std::path::PathBuf> = vec![
        // vulkan-rust-sys
        root.join("vulkan-rust-sys/src/handles.rs"),
        root.join("vulkan-rust-sys/src/enums.rs"),
        root.join("vulkan-rust-sys/src/bitmasks.rs"),
        root.join("vulkan-rust-sys/src/constants.rs"),
        root.join("vulkan-rust-sys/src/structs.rs"),
        root.join("vulkan-rust-sys/src/builders.rs"),
        root.join("vulkan-rust-sys/src/commands.rs"),
        root.join("vulkan-rust-sys/src/lib.rs"),
        // vulkan-rust wrappers
        root.join("vulkan-rust/src/generated/entry_wrappers.rs"),
        root.join("vulkan-rust/src/generated/instance_wrappers.rs"),
        root.join("vulkan-rust/src/generated/device_wrappers.rs"),
        root.join("vulkan-rust/src/generated/mod.rs"),
    ];
