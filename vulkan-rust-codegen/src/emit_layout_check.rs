...
pub fn emit_rust_layout_check(registry: &VkRegistry) -> String {
    ...
    for s in &registry.structs {
        ...
        let mut parts = vec![
            format!("size_of::<{rust_name}>()"),
            format!("align_of::<{rust_name}>()"),
        ];
        ...
        if !has_bitfields(s) {
            ...
            for field in &fields {
                parts.push(format!("offset_of!({rust_name}, {field})"));
            }
        }
...
mod tests {
    ...
    fn rust_output_includes_struct_and_offset_of() {
        ...
        assert!(rs.contains("size_of::<Extent2D>()"));
        assert!(rs.contains("offset_of!(Extent2D, width)"));
        assert!(rs.contains("offset_of!(Extent2D, height)"));
    }
    ...
    fn skips_keyword_fields_in_both_outputs() {
        ...
        assert!(!rs.contains("offset_of!(Foo, r#type)"));
        assert!(c.contains("offsetof(VkFoo, sType)"));
        assert!(rs.contains("offset_of!(Foo, s_type)"));
    }
    ...
    fn bitfield_structs_get_size_only() {
        ...
        assert!(rs.contains("size_of::<BitfieldStruct>()"));
        // No field offsets
